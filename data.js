...
const PROFILE = {
  ...
  intro: [
    ...
    "早起きが続かないからトラッカーを、家計簿が面倒だから自動登録を、バスケをもっと知りたいから図鑑を。動機はいつも自分の生活で、そこから実際に人へ納品するアプリまで広がりました。",
...
const PROJECTS = [
  ...
  },
  {
    ...
    emoji: "🌱",
    summary:
      "手書きの注文書を撮影するとAIが読み取り、商品マスタと照合して集計まで行う業務アプリ。自分以外の人のために作って納品した、はじめての実案件です。",
    body: [
      ...
      "使う人が業務の途中で迷わないよう、サイドバーを消して画面を一本道にし、和やかな緑基調のUIにしています。",
    ],
    ...
    highlights: ["手書き注文書をAIで読み取り", "商品マスタとのあいまい照合で表記ゆれを吸収", "複数枚の並列処理とHEIC対応", "実際の事業者へ納品"],
  },
  {
    id: "task-manager",
    title: "タスク管理 自動化",
...
const TIMELINE = [
  ...
  { date: "2026.09", title: "自分の外へ", note: "Instagram運用を開始し、前島園芸さまへ業務アプリを納品" },
];
...
const EXPERIENCE = [
  {
    role: "苗木注文書管理システムの開発・納品",
    org: "前島園芸さま",
    period: "2026.09",
    note: "手書きの注文書をAIで読み取って集計する業務アプリを、要件整理から納品まで担当。自分以外の人が毎日使うものを作った初めての仕事。",
  },
  {
...
const CAREER = {
  ...
  axis: "身体 × データ / AI",
  lead:
    "理学療法士の国家資格を取ったうえで、身体のことをデータで扱える場所に行きたいと考えています。いま14社を継続的に調べていて、大きく3つの方向で見ています。",
  groups: [
    {
      label: "スポーツ用品メーカー",
      note: "身体の知識をプロダクトに還元する方向。日系は秋冬インターンが本命、外資は通年でのウォッチが必要。",
      items: ["ミズノ", "アシックス", "NIKE", "adidas"],
    },
    {
      label: "身体 × AI のスタートアップ",
      note: "動作解析やリハビリ支援など、身体をデータで扱う領域。新卒一括採用が少なく、長期インターンとカジュアル面談が実質のルート。",
      items: ["CYBERDYNE", "Sportip", "SPLYZA", "Sapeet", "ジャパンヘルスケア", "Rehab for JAPAN", "ayumo"],
    },
    {
      label: "PT / OT 向けの人材サービス",
      note: "医療職のキャリアを支える側から業界全体を見る方向。",
      items: ["マイナビコメディカル", "エス・エム・エス（PTOT人材バンク）", "トライトグループ（PTOTSTワーカー）"],
    },
  ],
};
